...
export async function createDebitNoteFromInvoice(payload: {
  ...
  const resp = await api.post(ENDPOINTS.createDebitNote, payload);
  return resp.data;
}
...
export async function getAllDebitNotes(
  ...
  const resp: AxiosResponse = await api.get(ENDPOINTS.getAllDebitNotes, {
    params: { page, page_size },
  });
  return resp.data;
}
export async function getAllCreditNotes(
  ...
  const resp: AxiosResponse = await api.get(ENDPOINTS.getAllCreditNotes, {
    params: { page, page_size },
  });
  return resp.data;
}
