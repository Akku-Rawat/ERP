...
export const validatePO = (form: PurchaseOrderFormData) => {
  ...
  form.items.forEach((it, i) => {
    if (!it.itemCode) errors.push(`Item code missing at row ${i + 1}`);
    if (!it.quantity || it.quantity <= 0) errors.push(`Quantity invalid at row ${i + 1}`);
    if (!it.rate || it.rate <= 0) errors.push(`Rate invalid at row ${i + 1}`);
  });
