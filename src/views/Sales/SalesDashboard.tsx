  ...
  <div className="bg-white rounded-xl shadow p-8 flex flex-col items-center justify-center h-full">
    ...
    <div className="relative flex justify-center items-center mb-6" style={{ height: 220, width: 220 }}>
      ...
      <div className="absolute top-0 left-0 w-full h-full flex flex-col items-center justify-center">
        ...
        <span className="text-xl mt-2 text-gray-600 font-medium">New Customer</span>
      </div>
    </div>
...
const SalesDashboard: React.FC = () => {
  const [selectedRange, setSelectedRange] = React.useState<TimeRange>("This Month");
...
<div className="flex gap-8 mb-8">
  ...
  <div className="w-[60%]">
    <div className="bg-white rounded-xl shadow p-8 h-full">
      ...
      <div className="flex justify-end items-center mb-4">
        ...
        <span className="mr-3 text-md text-blue-600 font-bold">This Year</span>
        <span className="inline-block w-2 h-2 rounded-full bg-gray-400 mr-1"></span>
        <span className="text-md text-gray-500 font-bold">Last Year</span>
      </div>
  ...
  <table className="min-w-full">
    ...
    <tbody className="divide-y divide-gray-200">
      {topProducts.map((p) => (
        <tr key={p.name} className="hover:bg-indigo-50/70 cursor-pointer transition-colors duration-150">
          <td className="px-6 py-4 font-semibold text-gray-900">{p.name}</td>
          <td className="px-6 py-4 text-gray-700">{p.category}</td>
          <td className="px-6 py-4 text-indigo-700">{p.units}</td>
          <td className="px-6 py-4 font-semibold text-green-600">₹{p.revenue.toLocaleString()}</td>
        </tr>
      ))}
    </tbody>
  </table>
