...
import { Users, TrendingUp, AlertCircle, Calendar, CheckCircle, Clock, ChevronUp } from 'lucide-react';
...
export const ExpandedRowDetail: React.FC<ExpandedRowDetailProps> = ({ record, onCollapse }) => {
        ...
        <div className="max-w-6xl mx-auto">
          <div className="grid grid-cols-3 gap-8 mb-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-semibold">{record.designation}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">Grade:</span>
                  <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs font-bold rounded">{record.grade}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">Joining:</span>
                  <span className="text-sm font-semibold">{record.joiningDate}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">Bank:</span>
                  <span className="text-xs font-semibold">{record.bankAccount}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-slate-600">PF:</span>
                  <span className="text-xs font-semibold">{record.pfNumber}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl shadow-sm border border-green-200 p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-bold">₹{record.basicSalary.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-green-800">HRA:</span>
                  <span className="text-sm font-bold">₹{record.hra.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-green-800">Allowances:</span>
                  <span className="text-sm font-bold">₹{record.allowances.toLocaleString()}</span>
                </div>
                {record.arrears > 0 && (
            ...
            <div className="bg-gradient-to-br from-red-50 to-rose-50 rounded-xl shadow-sm border border-red-200 p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  <span className="text-sm text-red-800">Tax ({record.taxRegime}):</span>
                  <span className="text-sm font-bold">₹{record.taxDeduction.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-red-800">PF:</span>
                  <span className="text-sm font-bold">₹{record.pfDeduction.toLocaleString()}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-red-800">Other:</span>
                  <span className="text-sm font-bold">₹{record.otherDeductions.toLocaleString()}</span>
                </div>
          ...
          <div className="bg-white rounded-lg border border-slate-200 p-4 flex items-center justify-between">
            <div className="flex items-center gap-8">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">Working: <strong>{record.workingDays}</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <CheckCircle className="w-4 h-4 text-green-600" />
                <span className="text-sm">Paid: <strong>{record.paidDays}</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="w-4 h-4 text-slate-500" />
                <span className="text-sm">Created: <strong>{new Date(record.createdDate).toLocaleDateString()}</strong></span>
              </div>
            </div>
            <button onClick={onCollapse} className="text-sm text-blue-600 hover:text-blue-700 font-medium flex items-center gap-1">
