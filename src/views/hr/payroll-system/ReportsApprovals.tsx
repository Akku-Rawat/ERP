...
export const PayrollReports: React.FC<PayrollReportsProps> = ({ records }) => {
  const [tab, setTab] = useState<"summary" | "department" | "tax" | "compliance">("summary");
  ...
  const deptData = Object.values(
    paid.reduce((acc: Record<string, { dept: string; count: number; gross: number; net: number }>, r) => {
              ...
              {deptData.map(d => {
                ...
                return (
                  <div key={d.dept} className="flex flex-col items-center gap-2 flex-1">
                    ...
                    <span className="text-[10px] text-muted font-semibold text-center leading-tight">{d.dept}</span>
                  </div>
                );
              })}
            </div>
            <div className="flex items-center gap-4 justify-center">
              <span className="flex items-center gap-1.5 text-xs text-muted"><span className="w-3 h-3 rounded bg-primary/80 inline-block" /> Gross</span>
              <span className="flex items-center gap-1.5 text-xs text-muted"><span className="w-3 h-3 rounded bg-success/80 inline-block" /> Net</span>
            </div>
          </div>
            ...
            {[
              ...
              <div key={title} className={`rounded-xl border p-4 flex items-center gap-3 ${ok ? "border-success/30 bg-success/5" : "border-danger/30 bg-danger/5"}`}>
                ...
                <div>
                  ...
                  <p className={`text-lg font-extrabold ${ok ? "text-success" : "text-danger"}`}>{val}</p>
                </div>
              </div>
            ))}
          </div>
          <button onClick={() => alert("Downloading compliance report…")}
            className="flex items-center gap-2 px-4 py-2 border border-theme text-primary text-xs font-bold rounded-lg hover:bg-app transition">
