    ...
    <div className="flex items-center gap-3 p-3 rounded-xl border border-gray-100 bg-white">
      ...
      <div className="flex-1">
        <div className="flex items-start justify-between">
          ...
          <div className="ml-3">
            <div
              className={`text-xs px-2 py-1 rounded-md ${accent} text-white/95`}
            >
              {" "}
            </div>
          </div>
  ...
  const items: ScheduleItem[] = [
    {
      id: "m1",
      time: "10:00 - 11:00",
      title: "Meeting with Director Board",
      subtitle: "Call by Directors",
      kind: "meeting",
    },
    {
      id: "m2",
      time: "11:30 - 12:00",
      title: "1:1 with Product",
      subtitle: "Product",
      kind: "meeting",
    },
    {
  ...
  return (
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-gray-100">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Don't forget your daily activity
          </p>
        </div>
        <select
          aria-label="month"
      ...
      <div className="flex items-center justify-between pb-3 mb-3">
        ...
        <button
          onClick={() => setSelectedIndex(Math.min(6, selectedIndex + 1))}
          className="px-2 py-1 text-gray-600"
        >
          ▶
        </button>
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-2">
            ...
            <span className="text-xs text-green-600 font-semibold flex items-center gap-1 bg-green-50 px-2 py-1 rounded-md">
              <FaArrowUp className="text-xs" /> From last month
            </span>
          </div>
        </div>
        <div className="flex items-center gap-1 text-xs">
          <span className="text-gray-400 capitalize">{timeRange}</span>
          ...
          <div className="grid grid-rows-4 grid-cols-7 gap-2">
            {matrix.flatMap((row, rIdx) =>
              row.map((val, cIdx) => (
                <div
                  key={`${rIdx}-${cIdx}`}
                  className={`w-8 h-8 rounded-lg ${colorFor(val)} shadow-sm`}
                />
              )),
            )}
          </div>
...
const HrDashboard: React.FC = () => {
  ...
  const headcountData = useMemo(
    () => fullHeadcountData[timeRange],
    [timeRange],
  );
  const attendanceWeekData = useMemo(
    () => fullAttendanceData[timeRange],
    [timeRange],
  );
  const monthlyAttendance = useMemo(
    () => fullMonthlyAttendance[timeRange],
    [timeRange],
  );

  return (
    <>
      <div className=" bg-app p-4 lg:p-6">
        <div className="max-w-[1800px] mx-auto space-y-5">
          {/* Header */}
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-teal-400 to-blue-500 flex items-center justify-center text-white text-lg font-bold shadow-md">
                JD
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                <FaSync
                  className={`text-gray-600 ${refreshKey > 0 ? "animate-spin" : ""}`}
                />
              </button>
              <button className="px-4 py-2.5 bg-teal-500 hover:bg-teal-600 text-white rounded-lg text-sm flex items-center gap-2 transition shadow-sm">
                <FaDownload /> Export
              </button>
            </div>
          </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  ...
                  <div className="flex items-end gap-2 mb-4">
                    ...
                    <span className="text-sm text-green-600 font-semibold mb-1 flex items-center gap-1">
                      <FaArrowUp className="text-xs" /> 2.44%
                    </span>
                  </div>
                ...
                <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
                  ...
                  <div className="space-y-3.5">
                    {recentPayroll.map((pay, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-400 to-purple-400 flex items-center justify-center text-white text-xs font-semibold">
                            {pay.avatar}
                          </div>
                          <div>
                        ...
                        <div className="text-right">
                          ...
                          <div
                            className={`text-xs font-semibold ${pay.status === "Success" ? "text-green-600" : "text-yellow-600"}`}
                          >
                            {pay.status}
                          </div>
                        </div>
                ...
                <div className="bg-gradient-to-br from-blue-100 via-blue-100 to-blue-50 rounded-2xl p-5 text-white shadow-lg">
                  <span className="mt-9 text-lg font-bold text-gray-900">
                    Dayoff Request
                  </span>
                    ...
                    {dayoffRequests.map((req, idx) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                          <button className="w-8 h-8 rounded-lg bg-red-500 text-white flex items-center justify-center hover:bg-red-600 transition shadow-sm">
                            ✕
                          </button>
                          <button className="w-8 h-8 rounded-lg bg-green-500 text-white flex items-center justify-center hover:bg-green-600 transition shadow-sm">
                            ✓
                          </button>
                        </div>
                      </div>
                    ...
                    {attendanceDetails.map((att, idx) => (
                      ...
                      >
                        <div className="flex items-center gap-3 flex-1">
                          <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 flex items-center justify-center text-white text-xs font-semibold flex-shrink-0">
                            {att.emp
                              .split(" ")
                              .map((n) => n[0])
                              .join("")}
                        ...
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-semibold flex-shrink-0 ${att.status === "Attend" ? "bg-green-100 text-green-700" : "bg-orange-100 text-orange-700"}`}
                        >
                          {att.status}
                        </span>
                      </div>
