...
import { Plus, Edit2, Trash2, Save, X, Calendar, AlertTriangle, Check } from "lucide-react";
...
let policiesStore: LeavePolicy[] = [
  {
    ...
    usedBy: 45,
    leaveTypes: [
      { id: "lt_1", name: "Annual Leave", code: "AL", accrual: "Monthly", quota: 24, carryForward: true, maxCarry: 12, isPaid: true, requiresApproval: true },
      { id: "lt_2", name: "Sick Leave", code: "SL", accrual: "Yearly", quota: 14, carryForward: false, maxCarry: 0, isPaid: true, requiresApproval: false },
      { id: "lt_3", name: "Maternity Leave", code: "ML", accrual: "One-time", quota: 90, carryForward: false, maxCarry: 0, isPaid: true, requiresApproval: true },
      { id: "lt_4", name: "Compassionate Leave", code: "CL", accrual: "On-demand", quota: 5, carryForward: false, maxCarry: 0, isPaid: true, requiresApproval: true },
    ]
  }
];

const getPolicies = () => [...policiesStore];
const createPolicy = (policy: LeavePolicy) => { policiesStore.push(policy); };
...
export default function LeavePolicyTab() {
  ...
  const handleDelete = (id: string) => {
    ...
    if (policy && policy.usedBy > 0) {
      alert(`Cannot delete! This policy is used by ${policy.usedBy} employees.`);
      return;
    }
    deletePolicy(id);
    refreshPolicies();
  ...
  return (
    <div className="max-w-7xl space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Leave Policies</h2>
          <p className="text-sm text-gray-600 mt-1">Manage leave types and accrual rules</p>
        </div>
        <button
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500">
            ...
            <span>📅 From: {new Date(policy.applicableFrom).toLocaleDateString()}</span>
      ...
      <div className="border-t pt-4">
        <div className="grid grid-cols-2 gap-4 text-sm">
          ...
          {policy.leaveTypes.length > 4 && (
            <p className="text-xs text-gray-500 col-span-2">+{policy.leaveTypes.length - 4} more types...</p>
          )}
        </div>
      </div>
    </div>
...
function PolicyModal({ policy, onSave, onClose }: {
  ...
  const handleSaveLeaveType = (leaveType: LeaveType) => {
    const existingIndex = formData.leaveTypes.findIndex(lt => lt.id === leaveType.id);
    if (existingIndex >= 0) {
      const updated = [...formData.leaveTypes];
      updated[existingIndex] = leaveType;
      setFormData({ ...formData, leaveTypes: updated });
    } else {
      setFormData({ ...formData, leaveTypes: [...formData.leaveTypes, leaveType] });
    }
      ...
      <div className="bg-white rounded-xl max-w-6xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="px-6 py-4 border-b flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">Define leave types and accrual rules</p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            <div className="col-span-2 space-y-6">
                ...
                {annualLeave && annualLeave.quota < 24 && (
                  <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    ...
                    <p className="text-xs text-amber-700">
                      Warning: Zambian Labour Law requires minimum 24 days annual leave per year
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-sm font-semibold text-gray-900">Leave Types</h4>
                  <button
                ...
                <div className="space-y-2">
                    ...
                    >
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-3 text-xs text-gray-600">
                          ...
                          <span>•</span>
                          <span className="font-medium">{leaveType.quota} days</span>
                          <span>•</span>
                          <span>{leaveType.isPaid ? "Paid" : "Unpaid"}</span>
                          <span>•</span>
                          <span>{leaveType.requiresApproval ? "Needs approval" : "No approval"}</span>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button
                  ...
                  {formData.leaveTypes.length === 0 && (
                    <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-lg">
                      ...
                      <p className="text-sm text-gray-600 mb-3">No leave types added yet</p>
                      <button
                        onClick={handleAddLeaveType}
              ...
              <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-lg border border-orange-200 p-4 sticky top-4">
                      ...
                      {annualLeave.carryForward && (
                        <div className="p-3 bg-blue-50 rounded border border-blue-200 mt-2">
                          <p className="text-xs text-gray-700">
                            <strong>Carry Forward:</strong> Up to {annualLeave.maxCarry} days to 2026
                          </p>
                        </div>
                      )}
                    </div>
                  ...
                  {formData.leaveTypes.length > 0 && (
                    <div className="pt-3 border-t">
                      ...
                      <div className="space-y-1">
                        <div className="flex justify-between text-xs">
                          <span className="text-gray-600">Total leave types</span>
                          <span className="font-medium">{formData.leaveTypes.length}</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-gray-600">Total quota/year</span>
                          <span className="font-medium text-purple-600">{totalQuota} days</span>
                        </div>
                        <div className="flex justify-between text-xs">
                          <span className="text-gray-600">Paid leave types</span>
                          <span className="font-medium">
                            {formData.leaveTypes.filter(lt => lt.isPaid).length}
                          </span>
                        </div>
                      </div>
                    </div>
                ...
                <div className="mt-4 p-3 bg-amber-50 rounded border border-amber-200">
                  <p className="text-xs text-amber-800">
                    <strong>⚠️ Zambian Law:</strong> Minimum 24 days annual leave required
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg max-w-lg w-full p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-medium text-gray-700 mb-1">Accrual Method *</label>
              <select
                value={formData.accrual}
                onChange={(e) => setFormData({ ...formData, accrual: e.target.value as any })}
                className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
              >
            ...
            <div className="col-span-2">
              <label className="block text-xs font-medium text-gray-700 mb-1">Annual Quota (Days) *</label>
              <input
                type="number"
          ...
          <div className="border-t pt-4 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <label className="text-sm font-medium text-gray-900">Carry Forward</label>
                <p className="text-xs text-gray-500">Allow unused days to carry to next year</p>
              </div>
              <input
                type="checkbox"
            ...
            {formData.carryForward && (
              <div className="pl-4">
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, maxCarry: parseInt(e.target.value) || 0 })}
                  placeholder="12"
                  min="0"
                  className="w-full px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                />
            ...
            <div className="flex items-center justify-between">
              <div>
                <label className="text-sm font-medium text-gray-900">Paid Leave</label>
                <p className="text-xs text-gray-500">Employee receives salary during leave</p>
              </div>
              <input
                type="checkbox"
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-gray-500">Manager approval needed before leave</p>
              </div>
              <input
                type="checkbox"
...
function DeleteConfirmModal({ policy, onConfirm, onCancel }: {
  policy: LeavePolicy;
  onConfirm: () => void;
  onCancel: () => void;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div>
            <h4 className="text-lg font-semibold text-gray-900 mb-1">Delete Leave Policy?</h4>
        ...
        {policy.usedBy > 0 ? (
          <div className="p-3 bg-red-50 border border-red-200 rounded-lg mb-4">
            <p className="text-sm text-red-700">
              ⚠️ Cannot delete! This policy is used by <strong>{policy.usedBy} employees</strong>. 
          ...
          <div className="p-3 bg-gray-50 rounded-lg mb-4">
            <p className="text-sm text-gray-700">
              This action cannot be undone. The policy and all {policy.leaveTypes.length} leave types will be permanently deleted.
            </p>
          </div>
        )}
