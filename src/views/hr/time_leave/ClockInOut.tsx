...
interface ClockInOutProps {
  ...
  onClockOut: (data: { employeeId: string; timestamp: string }) => Promise<void>;
}

const ClockInOut: React.FC<ClockInOutProps> = ({ employeeId, onClockIn, onClockOut }) => {
  const [loading, setLoading] = useState(false);
  const [lastAction, setLastAction] = useState<"in" | "out" | null>(null);
//   const { toast } = useToast();

  // Check current status on mount
  const [status, setStatus] = useState<"checking" | " clocked-in" | "clocked-out">("checking");
    ...
    <div className="bg-card border border-[var(--border)] rounded-xl p-6 flex flex-col items-center gap-4">
      ...
      <div className="text-sm text-muted">
        {status === "checking" ? (
          ...
          <>
            Status: <span className={`font-medium ${status === "clocked-in" ? "text-success" : "text-muted"}`}>{status}</span>
            {lastAction && (
              <span className="ml-2">
