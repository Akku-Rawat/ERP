      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-slate-200">
            {records.map(record => {
              const bonusTotal = (record.bonuses || []).reduce((s, b) => s + (b.amount || 0), 0);
              return (
                <tr key={record.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      {(record.bonuses && record.bonuses.length > 0) && (
                        <div className="mt-2 text-xs">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs bg-teal-50 text-teal-700 px-2 py-0.5 rounded-full font-medium">+ ₹{bonusTotal.toLocaleString()}</span>
                          </div>
                          <div className="mt-1 space-y-1">
                            {record.bonuses!.map(b => (
                              <div key={b.id} className="flex items-center justify-between text-xs text-slate-600">
                                <div>{b.label}{b.approved ? ' • approved' : ' • pending'}</div>
                  ...
                  <td className="py-4 px-6 text-right font-semibold text-slate-800">
                    ...
                    {bonusTotal > 0 && (
                      <div className="text-xs text-slate-400 mt-1">Includes bonus: ₹{bonusTotal.toLocaleString()}</div>
                    )}
                  </td>
                    ...
                    <div className="flex gap-2 justify-center">
                      ...
                      {onAddBonus && record.status !== 'Paid' && (
                        <button
                          ...
                          title="Add Bonus"
                        >
                          <Plus className="w-4 h-4" /> <span className="hidden sm:inline">+ Bonus</span>
                        </button>
                      )}
                    </div>
                  </td>
