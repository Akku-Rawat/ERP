    ...
    <Card title="Statutory Compliance">
      <div className="space-y-3">
        {statutoryItems.map((s) => (
          <div key={s.id} className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <span className={`px-3 py-1 text-xs rounded-full ${s.status === 'Pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-green-100 text-green-800'}`}>
                {s.status}
              </span>
        ...
        <div className="flex justify-end">
          <button className="px-3 py-2 bg-indigo-600 text-white text-sm rounded-lg">Open statutory calendar</button>
        </div>
      </div>
    </Card>
  );
