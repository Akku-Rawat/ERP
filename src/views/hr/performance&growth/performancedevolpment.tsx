...
const PerformanceDevelopment: React.FC = () => {
  const [tab, setTab] = useState<'performance' | 'training' | 'appraisals'>('performance');
const screens = {
  ...
  useEffect(() => { console.log('PerformanceDevelopment active tab:', tab); }, [tab]);
        ...
        <div className="flex gap-8 border-b border-gray-300 pb-4 overflow-x-auto">
          <button
            ...
            className={`flex items-center gap-2 text-lg font-semibold transition pb-2 border-b-4 whitespace-nowrap ${
              tab === 'performance' ? 'text-indigo-600 border-indigo-500' : 'text-gray-500 border-transparent hover:text-indigo-600'
            }`}
          >
          ...
          <button
            ...
            className={`flex items-center gap-2 text-lg font-semibold transition pb-2 border-b-4 whitespace-nowrap ${
              tab === 'training' ? 'text-indigo-600 border-indigo-500' : 'text-gray-500 border-transparent hover:text-indigo-600'
            }`}
          >
          ...
          className={`flex items-center gap-2 text-lg font-semibold transition pb-2 border-b-4 whitespace-nowrap ${
            tab === 'appraisals' ? 'text-indigo-600 border-indigo-500' : 'text-gray-500 border-transparent hover:text-indigo-600'
          }`}>
            <FaMedal /> Appraisals
          </button> 
        </div>
