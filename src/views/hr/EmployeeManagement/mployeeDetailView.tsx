...
import { Eye, Download, Upload, Mail, Phone, MapPin, Calendar, Briefcase, DollarSign, FileText, X } from "lucide-react";
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
      <div className="bg-card w-full max-w-md rounded-xl shadow-xl">
        ...
        <div className="p-5 space-y-4">
          <div>
            <label className="text-xs font-medium text-muted">Document Description</label>
            <input
              value={description}
          ...
          <label className="block">
            <div className="border-2 border-dashed border-border rounded-lg p-5 text-center cursor-pointer hover:border-primary transition">
              ...
              <p className="text-xs text-muted">Click to upload or drag & drop</p>
              <p className="text-[11px] text-muted/70 mt-1">PDF, JPG, PNG (max 5MB)</p>
            </div>
            <input
              type="file"
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t border-border bg-background">
          <button onClick={onClose} className="px-4 py-1.5 text-xs border border-border rounded-lg hover:bg-row-hover">
            Cancel
          </button>
          <button
...
const EmployeeDetailView: React.FC<Props> = ({ employee, onBack, onDocumentUploaded }) => {
  const [activeTab, setActiveTab] = useState<'personal' | 'employment' | 'compensation' | 'documents'>('personal');
  ...
  const getStatusBadge = () => {
    ...
    if (statusLower === 'active') return 'bg-green-100 text-green-700 border-green-300';
    return 'bg-gray-100 text-gray-600 border-gray-300';
  };

  const handleUploadDocument = async ({ description, file }: { description: string; file: File }) => {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-muted text-xs">{employmentInfo?.JobTitle} • {employmentInfo?.Department}</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 gap-4">
          ...
          <div className="col-span-12 lg:col-span-3">
            <div className="bg-card rounded-lg shadow border border-border overflow-hidden sticky top-4">
              ...
              <div className="px-4 py-3 space-y-2.5">
                <QuickInfo icon={<Mail className="w-4 h-4" />} label="Email" value={contactInfo?.workEmail} />
                <QuickInfo icon={<Phone className="w-4 h-4" />} label="Phone" value={contactInfo?.phoneNumber} />
                <QuickInfo icon={<MapPin className="w-4 h-4" />} label="Location" value={employmentInfo?.workLocation} />
                <QuickInfo icon={<Calendar className="w-4 h-4" />} label="Joined" value={employmentInfo?.joiningDate} />
              </div>
              ...
              <div className="px-4 pb-4">
                <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg p-3 border border-green-200 dark:border-green-800">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    <p className="text-[10px] text-green-700 dark:text-green-400 font-bold uppercase">Gross Salary</p>
                  </div>
                  <p className="text-xl font-bold text-green-900 dark:text-green-100">
                    {payrollInfo?.currency} {Number(payrollInfo?.grossSalary || 0).toLocaleString()}
                  </p>
                  <p className="text-[10px] text-green-700 dark:text-green-400">{payrollInfo?.paymentFrequency}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-9">
            ...
            <div className="bg-card rounded-t-lg border border-border border-b-0 px-4 pt-3 flex gap-1 overflow-x-auto">
              {[
                { id: 'personal', label: 'Personal', icon: <FileText className="w-3.5 h-3.5" /> },
            ...
            <div className="bg-card rounded-b-lg rounded-tr-lg border border-border shadow">
              <div className="p-5" style={{ maxHeight: 'calc(100vh - 220px)', overflowY: 'auto' }}>
                  ...
                  <div className="space-y-5">
                    <Section title="Personal Information">
                      <div className="grid grid-cols-3 gap-x-4 gap-y-3">
                        <InfoField label="Full Name" value={`${personalInfo?.FirstName} ${personalInfo?.OtherNames || ''} ${personalInfo?.LastName}`} className="col-span-2" />
                    ...
                    <Section title="Contact Information">
                      <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                        ...
                        <InfoField label="Address" value={`${contactInfo?.address?.street}, ${contactInfo?.address?.city}`} className="col-span-2" />
                      </div>
                    </Section>
                    ...
                    <Section title="Compliance IDs">
                      <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                        ...
                        <InfoField label="NAPSA" value={identityInfo?.SocialSecurityNapsa} />
                        <InfoField label="NHIMA" value={identityInfo?.NhimaHealthInsurance} />
                      </div>
                    </Section>
                  </div>
                )}
                  ...
                  <div className="space-y-5">
                    <Section title="Employment Details">
                      <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                        ...
                        <InfoField label="Manager" value={employmentInfo?.reportingManager} />
                        <InfoField label="Joining Date" value={employmentInfo?.joiningDate} />
                        <InfoField label="Probation" value={employmentInfo?.probationPeriod} />
                        <InfoField label="Contract End" value={employmentInfo?.contractEndDate} />
                        <InfoField label="Shift" value={employmentInfo?.shift} />
                        <InfoField label="Work Address" value={employmentInfo?.workAddress} className="col-span-2" />
                      </div>
                    </Section>
                    ...
                    {employmentInfo?.weeklySchedule && (
                      <Section title="Weekly Schedule">
                        <div className="grid grid-cols-4 gap-x-4 gap-y-2">
                          <InfoField label="Mon" value={employmentInfo.weeklySchedule.monday || '—'} />
                          <InfoField label="Tue" value={employmentInfo.weeklySchedule.tuesday || '—'} />
                          <InfoField label="Wed" value={employmentInfo.weeklySchedule.wednesday || '—'} />
                          <InfoField label="Thu" value={employmentInfo.weeklySchedule.thursday || '—'} />
                          <InfoField label="Fri" value={employmentInfo.weeklySchedule.friday || '—'} />
                          <InfoField label="Sat" value={employmentInfo.weeklySchedule.saturday || '—'} />
                          <InfoField label="Sun" value={employmentInfo.weeklySchedule.sunday || '—'} />
                        </div>
                      </Section>
                    )}
                    ...
                    {leaveInfo && (
                      <Section title="Leave Setup">
                        <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                          <InfoField label="Opening Balance" value={leaveInfo?.openingLeaveBalance} className="col-span-2" />
                          <InfoField label="Monthly Rate" value={`${leaveInfo?.initialLeaveRateMonthly} days/month`} />
                          <InfoField label="Ceiling" value={`${leaveInfo?.ceilingAmount} days (${leaveInfo?.ceilingYear})`} />
                        </div>
                      </Section>
                    )}
                  </div>
                  ...
                  <div className="space-y-5">
                    <Section title="Salary Breakdown">
                      <div className="space-y-2">
                        {Object.entries(payrollInfo?.salaryBreakdown || {}).map(([key, value]) => (
                          <div key={key} className="flex justify-between items-center py-1.5 border-b border-border">
                            ...
                            <span className="text-xs font-bold text-main">
                              {payrollInfo?.currency} {Number(value).toLocaleString()}
                            </span>
                          </div>
                        ))}
                        <div className="flex justify-between items-center py-2 bg-background rounded-lg px-3 mt-2">
                          <span className="text-sm text-main font-bold">Gross Salary</span>
                    ...
                    <Section title="Statutory Deductions">
                      <div className="space-y-2">
                        ...
                        <DeductionRow label="PAYE" value={`${payrollInfo?.currency} ${Number(payrollInfo?.statutoryDeductions?.payeAmount || 0).toLocaleString()}`} />
                      </div>
                    </Section>

                    <Section title="Bank Account">
                      <div className="grid grid-cols-2 gap-x-4 gap-y-3">
                        <InfoField label="Account Name" value={payrollInfo?.bankAccount?.AccountName} />
                        <InfoField label="Account Number" value={payrollInfo?.bankAccount?.AccountNumber} />
                        <InfoField label="Bank Name" value={payrollInfo?.bankAccount?.BankName} />
                        <InfoField label="Branch Code" value={payrollInfo?.bankAccount?.branchCode} />
                        <InfoField label="Account Type" value={payrollInfo?.bankAccount?.AccountType} />
                      </div>
                    </Section>
                  </div>
                )}
                ...
                {activeTab === 'documents' && (
                  <div>
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="text-xs font-bold text-main uppercase tracking-wider">Documents</h3>
                      <button 
                    ...
                    {documents && documents.length > 0 ? (
                          ...
                          <div key={doc.id} className="flex items-center justify-between p-3 border border-border rounded-lg hover:bg-background transition-colors">
                            <div className="flex items-center gap-3 flex-1 min-w-0">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-xs font-semibold text-main truncate">{doc.description}</p>
                      ...
                      <div className="text-center py-12">
                        ...
                        <p className="text-muted text-xs font-semibold mb-1">No documents uploaded</p>
                        <p className="text-muted/70 text-[10px]">Click upload to add documents</p>
                      </div>
                    )}
                  </div>
                )}
...
const Section = ({ title, children }: any) => (
  <div>
    <h3 className="text-xs font-bold text-main uppercase tracking-wider mb-3 pb-2 border-b border-border">{title}</h3>
    {children}
  </div>
);
...
const InfoField = ({ label, value, className = "" }: any) => (
  <div className={className}>
    <p className="text-[10px] text-muted font-semibold mb-0.5 uppercase tracking-wide">{label}</p>
