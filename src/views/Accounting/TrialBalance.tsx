  ...
  <div className="space-y-6">
    ...
    <table className="w-full overflow-x-auto bg-white rounded-lg shadow-sm border border-gray-200">
      <thead className="bg-gray-50 border-b border-gray-200">
        <tr>
          <th className="px-6 py-3 text-left text-xs font-semibold text-gray-600 uppercase">Account Code</th>
      ...
      <tbody>
        {trialBalance.map((account: TrialBalanceAccount, idx: number) => (
          <tr key={idx} className="hover:bg-gray-50 border-b border-gray-100">
            ...
            <td className="px-6 py-4 text-sm text-right font-medium">{account.debit > 0 ? account.debit.toLocaleString() : "-"}</td>
            <td className="px-6 py-4 text-sm text-right font-medium">{account.credit > 0 ? account.credit.toLocaleString() : "-"}</td>
          </tr>
        ))}
        ...
        <tr className={`${totalDebit === totalCredit ? "bg-green-50" : "bg-red-50"} font-semibold`}>
          <td className="px-6 py-4 text-sm" colSpan={2}>{totalDebit === totalCredit ? "Balanced" : "Out of Balance"}</td>
          <td className="px-6 py-4 text-sm text-right" colSpan={2}>{Math.abs(totalDebit - totalCredit).toLocaleString()}</td>
        </tr>
      </tbody>
    </table>
  </div>
