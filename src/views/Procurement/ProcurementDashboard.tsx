...
import {
  FaTruck, FaCheckCircle, FaExclamationTriangle, FaClipboardList, FaMoneyCheckAlt, FaChevronUp, FaChevronDown
} from "react-icons/fa";
  ...
  <div className="space-y-8">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      {dashboardData.stats.map((stat, i) => (
        <div key={i} className={`rounded-xl shadow-md p-5 flex flex-col items-center justify-center ${stat.bg} hover:shadow-lg transition`}>
          <div className="mb-2">{stat.icon}</div>
          <div className="text-3xl font-bold flex items-center gap-2">
            {stat.value}
            {stat.delta !== undefined && (
              <span className={`flex items-center text-xs font-semibold ${stat.delta > 0 ? "text-red-500" : stat.delta < 0 ? "text-green-600" : "text-gray-400"}`}>
  ...
  {dashboardData.spendByCategory.map((entry, idx) => (
    <Cell key={entry.category} fill={COLORS[idx % COLORS.length]} />
      ...
      <div className="space-y-7">
              ...
              {dashboardData.approvalBottlenecks.map((row, i) => (
                <li key={i} className="flex items-center justify-between">
                  <div className={`flex items-center gap-2`}>
                    <span className={`inline-block rounded-full w-7 h-7 flex items-center justify-center font-bold uppercase ${row.color}`}>
                      {row.user.split(" ").map(x => x[0]).join("")}
                    </span>
                    <span>{row.user}</span>
                  </div>
        ...
        <div className="bg-white rounded-xl p-6 shadow border">
          <div className="font-semibold text-lg mb-2">Top Suppliers This Quarter</div>
          <ul className="space-y-2">
