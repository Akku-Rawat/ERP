...
const ItemModal: React.FC<ItemModalProps> = ({
  ...
  // Handlers
  const addNonExportRow = () =>
    setNonExportRows([
      ...nonExportRows,
      { tax: "", code: "", name: "", description: "", rate: "" },
    ]);
  const addExportRow = () =>
    setExportRows([
      ...exportRows,
  ...
  const deleteNonExportRow = (i: number) =>
    setNonExportRows(nonExportRows.filter((_, idx) => idx !== i));
  const deleteExportRow = (i: number) =>
    setExportRows(exportRows.filter((_, idx) => idx !== i));
  const deleteLocalRow = (i: number) =>
    setLocalRows(localRows.filter((_, idx) => idx !== i));

  const handleNonExportChange = (
    index: number,
    field: keyof TaxRow,
    value: string,
  ) => {
    setNonExportRows((rows) =>
      rows.map((row, i) => (i === index ? { ...row, [field]: value } : row)),
    );
  };
