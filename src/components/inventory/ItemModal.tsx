...
>
  ...
  {(["details", "taxDetails", "inventoryDetails"] as const).map((tab) => (
    <button
      key={tab}
      type="button"
      onClick={() => setActiveTab(tab)}
      className={`px-6 py-3 font-semibold text-sm rounded-t-lg ${
        activeTab === tab
          ? "text-primary bg-card shadow-sm"
          : "text-muted hover:bg-card/50"
      }`}
    >
      {tab === "details" && "Item Details"}
      {tab === "taxDetails" && "Tax Details"}
      {tab === "inventoryDetails" && "Inventory Details"}
    </button>
  ))}
</div>
