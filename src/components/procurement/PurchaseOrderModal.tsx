...
function SupplierDropdown({
  ...
  return (
    <div ref={ref} className={`relative w-full flex flex-col gap-1 ${className}`}>
      ...
      {open && !suppLoading && (
        <div className="absolute left-0 w-full mt-1 bg-white border shadow-lg rounded z-10" style={{ top: "100%" }}>
          <input
          ...
          <ul className="max-h-40 overflow-y-auto">
            ...
            {filtered.length === 0 && <li className="px-4 py-2 text-gray-500">No match</li>}
          </ul>
        </div>
      )}
    </div>
...
const PurchaseOrderModal: React.FC<PurchaseOrderModalProps> = ({ isOpen, onClose, onSubmit }) => {
  ...
  useEffect(() => {
    ...
    const loadSuppliers = async () => {
      try {
        ...
        const mockSuppliers = [{ name: "Supplier A" }, { name: "Supplier B" }, { name: "Supplier C" }];
  ...
  const reset = () => {
    ...
    setPaymentRows([{ paymentTerm: "", description: "", dueDate: "", invoicePortion: 0, paymentAmount: 0 }]);
            ...
            <section className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div className="grid grid-cols-3 gap-6 max-h-screen overflow-auto p-4">
                  <div className="col-span-2">
                    <div className="flex flex-col gap-4">
                      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <Input label="PO Number" name="poNumber" type="text" value={form.poNumber} onChange={handleForm} placeholder="Enter PO Number" className="w-full" />
                        <SupplierDropdown value={form.supplier} onChange={(name) => setForm((p) => ({ ...p, supplier: name }))} className="w-full" suppliers={suppliers} suppLoading={suppLoading} />
                        <Input label="Date" name="date" type="date" value={form.date} onChange={handleForm} className="w-full" />
                        ...
                        <div className="col-span-3 grid grid-cols-3 gap-4">
                          <Select label="Status" name="status" value={form.status} onChange={handleForm} options={[{ value: "Draft", label: "Draft" }, { value: "Submitted", label: "Submitted" }, { value: "Approved", label: "Approved" }, { value: "Cancelled", label: "Cancelled" }]} />
                          <Input label="Cost Center" name="costCenter" type="text" value={form.costCenter} onChange={handleForm} placeholder="Enter Cost Center" className="w-full" />
                          <Input label="Project" name="project" type="text" value={form.project} onChange={handleForm} placeholder="Enter Project Name" className="w-full" />
                        </div>
                      </div>
                    </div>
                    <div className="my-6 h-px bg-gray-600" />
                    {/* Order Items */}
                    <h3 className="mb-4 text-lg font-semibold text-gray-700 underline">Order Items</h3>
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-sm text-gray-600">
                        Showing {page * itemsPerPage + 1}–{Math.min((page + 1) * itemsPerPage, items.length)} of {items.length}
                      </span>
                      <div className="flex gap-1">
                        <button type="button" onClick={() => setPage(Math.max(0, page - 1))} disabled={page === 0} className="px-2 py-1 text-xs rounded bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">← Prev</button>
                        <button type="button" onClick={() => setPage(page + 1)} disabled={(page + 1) * itemsPerPage >= items.length} className="px-2 py-1 text-xs rounded bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">Next →</button>
                      </div>
                    </div>
                            ...
                            return (
                              <tr key={i} className="hover:bg-gray-50">
                                ...
                                <td className="px-1 py-1">
                                  <input className="w-full rounded border p-1 text-sm" name="itemCode" value={it.itemCode} onChange={(e) => handleItem(e, i)} />
                                </td>
                                <td className="px-1 py-1">
                                  <input type="date" className="w-full rounded border p-1 text-sm" name="requiredBy" value={it.requiredBy} onChange={(e) => handleItem(e, i)} />
                                </td>
                                <td className="px-1 py-1">
                                  <input type="number" className="w-full rounded border p-1 text-right text-sm" name="quantity" value={it.quantity} onChange={(e) => handleItem(e, i)} />
                                </td>
                                <td className="px-1 py-1">
                                  <input className="w-full rounded border p-1 text-sm" name="uom" value={it.uom} onChange={(e) => handleItem(e, i)} />
                                </td>
                                <td className="px-1 py-1">
                                  <input type="number" className="w-full rounded border p-1 text-right text-sm" name="rate" value={it.rate} onChange={(e) => handleItem(e, i)} />
                                </td>
                  ...
                  <div className="col-span-1 sticky top-0 flex flex-col items-center gap-6 px-4 lg:px-6 h-fit">
                    <div className="w-full max-w-sm space-y-6">
                      ...
                      {/* Summary */}
                      <div className="w-full max-w-sm rounded-lg border border-gray-300 p-4 bg-white shadow">
                        <h3 className="mb-3 text-lg font-semibold text-gray-700 underline">Order Summary</h3>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
              ...
              {activeTab === "tax" && (
                <div className="space-y-6">
                  <h3 className="mb-4 text-lg font-semibold text-gray-700 underline">Taxes and Charges</h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input label="Tax Category" name="taxCategory" type="text" value={form.taxCategory} onChange={handleForm} placeholder="Enter Tax Category" className="w-full" />
                    <Input label="Shipping Rule" name="shippingRule" type="text" value={form.shippingRule} onChange={handleForm} placeholder="Enter Shipping Rule" className="w-full" />
                    <Input label="Incoterm" name="incoterm" type="text" value={form.incoterm} onChange={handleForm} placeholder="Enter Incoterm" className="w-full" />
                  </div>

                  <Input label="Purchase Taxes and Charges Template" name="taxesChargesTemplate" type="text" value={form.taxesChargesTemplate} onChange={handleForm} placeholder="Enter Template" className="w-full" />
                  ...
                  <div>
                            ...
                            paginatedTaxRows.map((row, idx) => {
                              ...
                              return (
                                <tr key={i} className="hover:bg-gray-50">
                                  <td className="px-3 py-2 text-center">{i + 1}</td>
                                  <td className="px-1 py-1">
                                    <input className="w-full rounded border p-1 text-sm" name="type" value={row.type} onChange={(e) => handleTaxRowChange(i, "type", e.target.value)} />
                                  </td>
                                  <td className="px-1 py-1">
                                    <input className="w-full rounded border p-1 text-sm" name="accountHead" value={row.accountHead} onChange={(e) => handleTaxRowChange(i, "accountHead", e.target.value)} />
                                  </td>
                                  <td className="px-1 py-1">
                                    <input type="number" className="w-full rounded border p-1 text-sm" name="taxRate" value={row.taxRate} onChange={(e) => handleTaxRowChange(i, "taxRate", Number(e.target.value))} />
                                  </td>
                                  <td className="px-1 py-1">
                                    <input type="number" className="w-full rounded border p-1 text-sm" name="amount" value={row.amount} onChange={(e) => handleTaxRowChange(i, "amount", Number(e.target.value))} />
                                  </td>
                                  <td className="px-1 py-1 text-right font-medium">{total.toFixed(2)}</td>
                                  <td className="px-1 py-1 text-center">
                                    <button type="button" onClick={() => removeTaxRow(i)} className="p-1 text-red-600 hover:bg-red-50 rounded">
                                      <Trash2 className="w-4 h-4" />
                                    </button>
                                  </td>
                                </tr>
                    ...
                    <div className="flex items-center justify-between mt-3">
                      <span className="text-sm text-gray-600">
                        Showing {taxRows.length === 0 ? 0 : taxPage * taxItemsPerPage + 1}–{Math.min((taxPage + 1) * taxItemsPerPage, taxRows.length)} of {taxRows.length}
                      </span>
                      <div className="flex gap-1">
                        <button type="button" onClick={() => setTaxPage(Math.max(0, taxPage - 1))} disabled={taxPage === 0} className="px-2 py-1 text-xs rounded bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">← Prev</button>
                        <button type="button" onClick={() => setTaxPage(taxPage + 1)} disabled={(taxPage + 1) * taxItemsPerPage >= taxRows.length} className="px-2 py-1 text-xs rounded bg-gray-100 disabled:opacity-50 disabled:cursor-not-allowed">Next →</button>
                      </div>
                    </div>
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <TextArea label="Supplier Address" name="supplierAddress" value={form.supplierAddress} onChange={handleForm} rows={2} />
                      <TextArea label="Supplier Contact" name="supplierContact" value={form.supplierContact} onChange={handleForm} rows={2} />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <TextArea label="Dispatch Address" name="dispatchAddress" value={form.dispatchAddress} onChange={handleForm} rows={2} />
                      <TextArea label="Shipping Address" name="shippingAddress" value={form.shippingAddress} onChange={handleForm} rows={2} />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <TextArea label="Company Billing Address" name="companyBillingAddress" value={form.companyBillingAddress} onChange={handleForm} rows={2} />
                      <TextArea label="Place of Supply" name="placeOfSupply" value={form.placeOfSupply} onChange={handleForm} rows={2} />
                    </div>
                  </div>
                </div>
              )}
