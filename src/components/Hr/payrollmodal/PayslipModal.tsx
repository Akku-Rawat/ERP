...
export const PayslipModal: React.FC<PayslipModalProps> = ({
  record, onClose, onDownload, onEmail,
}) => {
  if (!record) return null;

  const totalDed = record.taxDeduction + record.pfDeduction + record.esiDeduction +
    record.professionalTax + record.otherDeductions;
  ...
  const earnRows = [
    ...
    ...(record.overtimePay > 0 ? [{ label: "Overtime Pay",  amt: record.overtimePay  }] : []),
    ...(record.totalBonus  > 0 ? [{ label: "Bonus",         amt: record.totalBonus   }] : []),
    ...
    <div className="fixed inset-0 z-50 bg-black/65 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-3 bg-slate-50 border-b border-slate-200">
          <span className="text-xs font-bold text-slate-500">Salary Slip · {period}</span>
          ...
          <div className="px-8 py-7" style={{ fontFamily: "system-ui,-apple-system,sans-serif" }}>
            ...
            <div className="flex items-start justify-between pb-4 border-b-2 border-slate-900 mb-5">
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-lg bg-primary flex items-center justify-center text-white font-extrabold text-sm shrink-0">I</div>
                <div>
                  <p className="text-sm font-extrabold text-slate-900 leading-none">Izyane InovSolutions Pvt. Ltd.</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">HR & Payroll Division</p>
                </div>
              </div>
              <div className="text-right">
                <p className="text-[9px] text-slate-400 uppercase tracking-widest">Payslip For The Month</p>
                <p className="text-sm font-extrabold text-slate-900">{period}</p>
              </div>
            </div>
            ...
            <div className="flex gap-5 mb-5">
              <div className="flex-1">
                <p className="text-[9px] font-extrabold uppercase tracking-widest text-slate-400 mb-2">Employee Summary</p>
                <table className="w-full">
                  <tbody>
                    {[
                      ...
                      <tr key={l}>
                        <td className="text-[11px] text-slate-500 py-[3px] pr-2 w-36 whitespace-nowrap">{l}</td>
                        <td className="text-[11px] text-slate-400 pr-2 w-3">:</td>
                        <td className="text-[11px] font-semibold text-slate-800">{v}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <table className="w-full text-[11px] border-collapse" style={{ border: "1px solid #e2e8f0" }}>
              <thead>
                <tr style={{ background: "#f8fafc" }}>
                  {["Earnings", "Amount", "Deductions", "Amount"].map((h, i) => (
                    <th
                      ...
                      style={{ border: "1px solid #e2e8f0", width: i % 2 === 0 ? "38%" : "12%" }}
                    >
                      {h}
                    </th>
                  ))}
            ...
            {/* Net payable */}
            <div className="flex items-center justify-between px-3 py-2.5 border border-t-0 border-slate-200 mb-3" style={{ background: "#f8fafc" }}>
              <div>
            ...
            <p className="text-right text-[10px] text-slate-500 mb-5">
              Amount In Words:{" "}
              <em className="not-italic font-semibold text-slate-700">{toWords(record.netPay)}</em>
            </p>
