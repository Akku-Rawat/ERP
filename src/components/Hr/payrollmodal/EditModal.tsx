...
export const EditModal: React.FC<EditModalProps> = ({
  record, onClose, onSave, onChange,
}) => {
  const [bonuses, setBonuses] = useState<Bonus[]>(record?.bonuses ?? []);
  const [addingBonus, setAddingBonus] = useState(false);
  const [newBonus, setNewBonus] = useState({ bonusType: "Performance", amount: 0, label: "" });
  ...
  const handleAddBonus = () => {
    ...
    onChange("totalBonus", updated.reduce((s, b) => s + b.amount, 0));
  ...
  const estimatedGross = record.basicSalary + record.hra + record.allowances + record.arrears +
    bonuses.reduce((s, b) => s + b.amount, 0) + (record.overtimePay ?? 0);
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="bg-primary px-6 py-5 text-white shrink-0">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-white/70 mt-0.5">{record.employeeName} · {record.employeeId}</p>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          {/* Salary fields */}
          <div>
            <p className="text-[10px] font-extrabold text-muted uppercase tracking-wider mb-3">Earnings</p>
            <div className="grid grid-cols-2 gap-4">
              <Field label="Basic Salary"  value={record.basicSalary}     onChange={v => onChange("basicSalary", v)}  />
              <Field label="HRA"           value={record.hra}             onChange={v => onChange("hra", v)}          />
              <Field label="Allowances"    value={record.allowances}      onChange={v => onChange("allowances", v)}   />
              <Field label="Arrears"       value={record.arrears}         onChange={v => onChange("arrears", v)}      helper="Pending salary arrears" />
              <Field label="Overtime Pay"  value={record.overtimePay}     onChange={v => onChange("overtimePay", v)}  />
              <Field label="Other Deductions" value={record.otherDeductions} onChange={v => onChange("otherDeductions", v)} />
            </div>
          </div>
          ...
          <div className="rounded-xl border border-theme bg-app p-4">
            ...
            {addingBonus && (
              <div className="bg-card border border-theme rounded-xl p-4 mb-3 space-y-3">
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-muted uppercase tracking-wider block mb-1">Type</label>
                    <select
                    ...
                    >
                      {BONUS_TYPES.map(bt => <option key={bt.value} value={bt.value}>{bt.label}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-[10px] font-bold text-muted uppercase tracking-wider block mb-1">Label</label>
                    <input
                      type="text"
                      placeholder="e.g. Q4 Performance"
                      value={newBonus.label}
                      onChange={e => setNewBonus(p => ({ ...p, label: e.target.value }))}
                      className="w-full px-3 py-2 border border-theme rounded-lg text-xs bg-app text-main focus:outline-none focus:border-primary"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setNewBonus(p => ({ ...p, amount: Number(e.target.value) }))}
            ...
            {bonuses.length === 0 ? (
              <p className="text-xs text-muted text-center py-3">No bonuses added</p>
            ) : (
              <div className="space-y-2">
                {bonuses.map(b => (
                  <div key={b.id} className="flex items-center gap-3 bg-card border border-theme rounded-lg p-3">
          ...
          <div className="rounded-xl bg-gradient-to-r from-primary/10 to-primary/5 border border-primary/20 p-4 flex items-center justify-between">
            <div>
              <p className="text-[10px] font-extrabold text-primary uppercase tracking-wider">Estimated Net Pay</p>
        ...
        <div className="shrink-0 border-t border-theme px-6 py-4 bg-app flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border border-theme text-main rounded-lg text-sm font-semibold hover:bg-card transition">
            Cancel
          </button>
          <button onClick={onSave} className="flex-1 py-2.5 bg-primary text-white rounded-lg text-sm font-bold hover:opacity-90 transition flex items-center justify-center gap-2">
