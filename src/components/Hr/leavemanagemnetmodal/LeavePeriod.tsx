...
export const LeavePeriod: React.FC<{ onAdd: () => void; onClose?: () => void }> = ({ onAdd, onClose }) => {
  return (
    <div className="flex">
      ...
      <div className="flex-1 bg-white rounded-xl border border-slate-200">
        <div className="p-6 flex items-center justify-between border-b border-slate-100 bg-white">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-semibold">Leave Period</h2>
          </div>
          <div>
            <button onClick={onAdd} className="px-4 py-2 bg-gray-200 text-gray-700 rounded">+ Add Leave Period</button>
          </div>
        </div>
        ...
        <div className="p-6 min-h-[280px] flex items-center justify-center text-slate-500">
          <div className="text-center">
            ...
            <div>You haven't created a Leave Period yet</div>
            <button onClick={onAdd} className="mt-3 px-3 py-1 bg-gray-50 border rounded">Create your first Leave Period</button>
          </div>
        </div>
      </div>
    </div>
