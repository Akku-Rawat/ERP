...
// --- Component ---
const InvoiceDefaultTemplate = forwardRef<HTMLDivElement, InvoiceDefaultTemplateProps>(
  ({ data, companyLogoUrl }, ref) => {
    ...
    // Layout
    return (
      <div ref={ref} className="max-w-[790px] min-h-[1120px] mx-auto bg-white shadow border p-0 font-sans text-[15px] relative" style={{ fontFamily: 'Inter, Arial, sans-serif' }}>
        {/* HEADER: Blue block */}
        <div
          className="flex justify-between items-start px-8 pt-7 pb-3"
          style={{ background: "#183A75", color: "white", borderTopLeftRadius: 10, borderTopRightRadius: 10 }}
        >
          <div>
            ...
            >
              {logo || companyLogoUrl ? (
                <img src={logo || companyLogoUrl} alt="Logo" className="h-14 max-w-full object-contain rounded-md" />
              ) : (
          ...
          {/* Business Details */}
          <div className="text-[16px] text-right leading-tight mt-2" style={{ minWidth: 240 }}>
            <div className="font-bold text-[18px] mb-2">Business Name</div>
            {[data.billingAddressLine1, data.billingAddressLine2, data.billingCity, data.billingState, data.billingPostalCode].filter(Boolean).map((line, i) => (
              <div key={i}>{line}</div>
            ))}
        ...
        <div className="flex flex-row px-8 py-6 border-b">
          <div className="flex-1 space-y-2">
            ...
            <div>Invoice # <span className="font-bold text-slate-700">{data.invoiceId || "0000"}</span></div>
            <div>Date of Issue <span className="font-bold text-slate-700">{data.dateOfInvoice || "MM/DD/YYYY"}</span></div>
            <div>Due Date <span className="font-bold text-slate-700">{data.dueDate || "MM/DD/YYYY"}</span></div>
          </div>
          <div className="flex-1">
            <div className="font-bold">BILL TO:</div>
            <div className="font-bold uppercase">{data.billToContact || "CUSTOMER NAME"}</div>
            {[data.billToAddress, data.billToCompany, data.billingCity, data.billingState, data.billingPostalCode].filter(Boolean).map((line, i) => (
            ...
            <tbody>
              {data.items.map((item, idx) => (
                <tr className="border-b last:border-b-0" key={idx}>
                  ...
                  <td className="text-right py-1">{symbol}{item.listPrice?.toLocaleString('en-IN', { minimumFractionDigits: 2 }) || "0.00"}</td>
                  <td className="text-right py-1">{symbol}{((item.quantity * item.listPrice - item.discount + item.tax) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="px-8 py-6 flex gap-6">
          <div className="flex-1">
            ...
            <div className="text-gray-600 mb-4 text-sm">{data.termsAndConditions || "Text Here"}</div>
            <div className="font-bold mt-5 mb-1">CONDITIONS/INSTRUCTIONS</div>
            <div className="text-gray-600 text-sm">{data.notes || "Text Here"}</div>
          </div>
          <div className="w-64">
            <div className="flex justify-between py-1">
              <span>Subtotal</span>
              <span>{symbol}{data.subTotal?.toLocaleString('en-IN', { minimumFractionDigits: 2 }) || "0.00"}</span>
            </div>
            <div className="flex justify-between py-1">
              <span>Discount</span>
              <span>-{symbol}{data.totalDiscount?.toLocaleString('en-IN', { minimumFractionDigits: 2 }) || "0.00"}</span>
            </div>
            <div className="flex justify-between py-1">
              <span>TaxRate</span>
              <span>5%</span>
            ...
            <div className="flex justify-between py-1">
              <span>Tax</span>
              <span>{symbol}{data.totalTax?.toLocaleString('en-IN', { minimumFractionDigits: 2 }) || "0.00"}</span>
            </div>
            <div className="flex justify-between border-t pt-3 font-bold text-lg">
              <span>TOTAL</span>
              <span>{symbol}{data.grandTotal?.toLocaleString('en-IN', { minimumFractionDigits: 2 }) || "0.00"}</span>
            </div>
          </div>
        </div>
